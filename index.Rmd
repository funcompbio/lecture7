...
* The R language is interpreted, vectorized and includes implicit memory
  management.  
  &nbsp;&nbsp;
* Looping statements are slow in R. The following example would be a suboptimal
...
* This would be the vectorized optimal approach in R:
  <pre>
  logsum <- sum(log(x))
  </pre>
...
* In this example, once we combine the city population with
  the absolute incidence, we can now calculate the
  [cumulative incidence](https://en.wikipedia.org/wiki/Incidence_%28epidemiology%29)
...
* Parameters `all`, `all.x` and `all.y` in the `merge()` function control
  ...
  &nbsp;&nbsp;
![](img/mergeJoins.jpg)
.footer[Image from [R for Dummies, _How to use the merge() function with data sets in R_.](https://www.dummies.com/book/technology/programming-web-design/r/r-for-dummies-2nd-edition-281846)]
  <pre>
